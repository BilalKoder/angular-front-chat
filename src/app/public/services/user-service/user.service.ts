...
export class UserService {
  ...
  findByUsername(username: string): Observable<UserI[]> {
    return this.http.get<UserI[]>(`http://172.16.200.109:3000/api/users/find-by-username?username=${username}`);
  }

  create(user: UserI): Observable<UserI> {
    return this.http.post<UserI>('http://172.16.200.109:3000/api/users', user).pipe(
